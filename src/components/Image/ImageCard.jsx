...
const ImageCard = ({image,type="def"}) => {
  ...
  if(type === "def"){
    ...
    hei = image.data.images[0].images.height
    wid = image.data.images[0].images.width
  }


  let cldImage = cld.image(type === "def" ? image.images.thumbId : image.data.images[0].images.thumbId)
  ...
  return (
    ...
    : filters.view === "large" ? ('h-[300px]') : 'h-[200px]'} relative top-0 flex-auto`}>
      {
        window.location.pathname === '/collections' && type === "def" && (
      ...
      }
      {
        type === "def" ? (
          ...
          // Render If its being called from purchases or my orders section
          <Link to={`/order/${image.data._id}`} className='flex-auto'>
          <div className={`${filters.view === "compact" ? ('h-[150px]') 
              : filters.view === "small" ? ('h-[100px]') 
              : filters.view === "large" ? ('h-[300px]') : 'h-[200px]'} flex-auto  rounded bg-emerald-200 relative top-0`}>
              <AdvancedImage className="h-[100%] w-full object-cover rounded cursor-pointer" cldImg={cldImage} plugins={[lazyload({rootMargin: '10px 20px 10px 30px', threshold: 0.25})]} />
              {
                ...
                image.data.images.length > 1 && (
                  <h6 className='absolute select-none bottom-[10px] right-[10px] rounded bg-white px-[10px]'>+{image.data.images.length - 1} other</h6>
                )
              }
          </div>
          </Link>  
